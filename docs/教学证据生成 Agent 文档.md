...
3. recorded_online_class：
   ...
   不得生成学生回答不足、等待学生回答不足
   或师生互动不足的证据。
   不得把“提问后很快进入教师说明”“教师自问自答”
   “请学生回答”“点名或举手”等真实课堂互动建议，
   放到 question_quality、learning_check_level、
   weakness_detection 等其他类别中变相输出。
   录播课中的提问句只能按自学提示、结构引导、
   讲解衔接或信息密度来分析。
...
录播课还必须进行语义级过滤。
即使 category 不是上述类别，只要卡片标题、事实、
解释、建议、原文引用或 teacherView 中出现以下含义，
也必须拒绝：
...
Teaching Evidence Agent 必须采用两阶段结构：

```text
课堂证据
  ↓
Evidence Analyst：内部分类、统计口径、证据强度判断
  ↓
Teacher Coach：转译为教师可读的课堂观察与可操作建议
  ↓
Guardrail：检查是否外显系统规则、语气是否过硬、建议是否空泛
```

### 11.1 内部分析字段

内部分析用于系统判断，不直接展示给教师。允许写入：
...
```json
{
  "analysis": {
    ...
    "utteranceType": "task_instruction",
...
- `title` 必须说明是哪一类证据，例如“课堂组织与任务切换”“提问后很快进入教师说明”“从例题中归纳方法”，不得只写“教学证据”。
- `observation` 描述片段中真实发生的行为，必须锚定时间、原句或课堂动作。
- `teachingMeaning` 可以解释这个片段对教学复盘有什么价值，但不得解释系统如何分类或计数。
- `nextStep` 必须给出与当前片段直接对应的教师动作。
- `exampleWording` 尽量给出一句教师可直接使用的话术。
...
内部判断可以写在 `analysis.internalReason`，但教师端必须转译为课堂观察。
...
```json
{
  ...
  "includedInQuestionCount": false,
  "includedInLearningCheckCount": false,
  "internalReason": "该表达用于维持秩序、安排任务或切换流程"
}
```
...
```text
课堂观察：在这一片段中，教师通过“请安静”“翻到第28页”“小组开始”等指令，快速组织学生从讲解转入练习。

可尝试的调整：在操作指令后补充一个与学习目标有关的关注点，例如：“先比较第3题和刚才例题的条件有什么不同，再开始讨论。”这样既能完成课堂组织，也能帮助学生明确练习时需要运用的思路。
```

方法归纳教师展示版：

```text
课堂观察：教师在完成例题后，将解题过程归纳为“先判断分母是否相同；分母不同时先通分，再比较分子”，帮助学生从一道题中提炼出可重复使用的步骤。

可继续观察：接下来可以提供一道分母不存在倍数关系的新题，请学生先说明准备采用什么方法，再完成计算。例如：“这道题还能直接比较分子吗？你准备先做哪一步，为什么？”
```

如果系统无法提取具体方法，建议必须保持保守，不得使用没有指向的“该方法”：

```text
教师已经开始从当前例题中归纳解题步骤。可以在归纳后安排一道条件略有变化的练习，并请学生先说出准备采用的步骤和理由，再开始作答。
```
...
- `positive`：积极证据，说明课堂中的亮点或值得保留的做法；
- `neutral`：中性观察，描述课堂事实和可继续观察的方向；
- `negative`：需优化证据，指出可能影响学生表达、理解检查或课堂推进的问题。

教师端展示规则：

- 积极证据：点出亮点，语气可以适度肯定，但不夸大，不写成泛泛表扬；
- 中性证据：保持观察语气，说明这段内容说明了什么；
- 需优化证据：给出温和、具体、可操作的建议，不使用责备式表达。

建议长度控制：

- `observation`：1句；
- `teachingMeaning`：1句以内；
- `nextStep`：1句，必须对应到该片段原文或课堂动作。

示例：

```text
积极：教师把“补短”作为解决这类题的步骤提炼出来，有助于学生把当前例题转化为可复用方法。后续可以让学生用这个方法处理一道条件略变的题，并先说出选择依据。

中性：这一段主要是在组织学生进入下一任务。可以在指令后补一句学习关注点，让学生知道接下来要观察或比较什么。

需优化：教师提出问题后很快继续说明，学生可组织回答的时间较短。如果希望听到学生思路，可以先邀请一名学生回答，并留出几秒等待。
```
