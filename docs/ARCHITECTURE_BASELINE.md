...
> 文档状态：历史长期标杆。它保留了早期对完整商业化系统的架构想象，但不再代表当前 M1 的执行范围。当前 M1 不做登录、多租户、视频 OCR、课件分析、强制整节课翻译和预置组织字段；若与 `TECHNICAL_MANUAL.md` 冲突，以 `TECHNICAL_MANUAL.md` 为准。
